...
class TexasHumanAgent(Agent):
    ...
    def eval_step(self, state):
        ...
        public_cards = [card.get_index() for card in state.public_cards]
        ...
        # Current State
        print(">>> Current Round:{} // Public Cards:{} // Pot:{}(Sum:{})".format(
            current_round, public_cards, state.pot, sum(state.pot)))
        print(">>> Your Hand:{}".format([card.get_index() for card in state.hand_cards]))
...
class LimitTexasHumanAgent(Agent):
    '''
    A Human agent.
...
class ToyPokerHumanAgent(Agent):
    '''
    A Human agent.
