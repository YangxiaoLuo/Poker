...
class PokerState:
    ...
    def get_lossless_abstraction(self):
        ...
        Returns:
            (list): strings of cards' ranks and suits. First 2 are hand cards.
        '''
        hand_cards = [card.get_index() for card in self.hand_cards]
        public_cards = [card.get_index() for card in self.public_cards]
        return PokerState.get_suit_normalization(hand_cards, public_cards)

    # @classmethod
    # def get_suit_normalization(hand_cards, public_cards):
    #     '''
    ...
    #     sorted_cards = sorted(hand_cards, key=lambda card: LookUpStr.RANK.index(card.rank), reverse=True) \
    #         + sorted(public_cards, key=lambda card: LookUpStr.RANK.index(card.rank), reverse=True)
